...
from deepsvg.model.config import _BaseModelConfig
from deepsvg.model.layers.improved_transformer import *
from deepsvg.model.layers.positional_encoding import PositionalEncodingLUT, PositionalEncodingSinCos
from deepsvg.model.layers.transformer import *
from deepsvg.model.utils import (
...
from deepsvg.utils.utils import (
    _make_batch_first,
    _make_seq_first,
    _pack_group_batch,
    _unpack_group_batch,
...
class ConstEmbedding(nn.Module):
    def __init__(self, cfg: _BaseModelConfig, seq_len):
        super().__init__()
...
class LabelEmbedding(nn.Module):
    def __init__(self, cfg: _BaseModelConfig):
        super().__init__()
    ...
    def forward(
        ...
        if return_tgt and self.cfg.self_match:  # Assignment
            ...
            args_logits = torch.gather(
                args_logits,
                dim=1,
                index=assignment.unsqueeze(-1).expand_as(args_logits),
            )
...
if __name__ == "__main__":
    ...
    x_dumb = torch.randn(16, 1, 28, 28, dtype=torch.float)
    y_dumb = model(x_dumb)
    print(x_dumb.shape)
    print(y_dumb.shape)
