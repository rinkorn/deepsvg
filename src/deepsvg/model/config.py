...
class _BaseModelConfig:
    ...
    def get_model_args(self):
        ...
        if self.rel_targets:
            model_args += (
                ["commands_grouped", "args_rel_grouped"] if self.decode_stages == 1 else ["commands", "args_rel"]
            )
        else:
...
class SketchRNN(_BaseModelConfig):
...
class Sketchformer(_BaseModelConfig):
...
class OneStageOneShot(_BaseModelConfig):
...
class Hierarchical(_BaseModelConfig):
...
class HierarchicalSelfMatching(_BaseModelConfig):
