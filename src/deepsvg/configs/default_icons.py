...
class ExperimentConfig(_BaseExperimentConfig):
    ...
    def set_train_vars(self, train_vars, dataloader):
        train_vars.x_inputs_train = [
            dataloader.dataset.get(idx, [*self.model_args, "tensor_grouped"])
    ...
    def visualize(self, model, output, train_vars, step, epoch, summary_writer, visualization_dir):
        ...
        for i, data in enumerate(train_vars.x_inputs_train):
            ...
            try:
                svg_path_sample = (
                    SVG.from_tensor(tensor_pred.data, viewbox=Bbox(256), allow_empty=True)
                    .normalize()
                    .split_paths()
                    .set_color("random")
                )
            ...
            svg_path_gt = (
                SVG.from_tensor(tensor_target.data, viewbox=Bbox(256)).normalize().split_paths().set_color("random")
            )
            img = make_grid([svg_path_sample, svg_path_gt]).draw(
                do_display=False, return_png=True, fill=False, with_points=False
            )
