...
from deepsvg.configs._BaseExperimentConfig import _BaseExperimentConfig
from deepsvg.logger import get_logger
from deepsvg.utils import Stats, Timer, TrainVars
...
def train(cfg: _BaseExperimentConfig, model_name, experiment_name="", log_dir="./logs", debug=False, resume=False):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    print("Parameters")
    cfg.print_params()
    print("Model parameters")
    cfg.model_cfg.print_params()

    print("Loading dataset")
    LOGGER.info(cfg.dataloader_module)
    load_dataset = importlib.import_module(cfg.dataloader_module).load_dataset
    ...
    if resume:
        ckpt_exists = utils.load_ckpt_list(
            checkpoint_dir, model, None, optimizers, scheduler_lrs, scheduler_warmups, stats, train_vars
        )
    ...
    if resume and ckpt_exists:
        ...
        single_foward_dataloader = DataLoader(
            dataset,
            batch_size=cfg.batch_size // cfg.num_gpus,
            shuffle=True,
            drop_last=True,
            num_workers=cfg.loader_num_workers,
            collate_fn=cfg.collate_fn,
        ...
        model_args, params_dict = (
            [data[arg].to(device) for arg in cfg.model_args],
            cfg.get_params(0, 0),
        )
        ...
        for n_iter, data in enumerate(dataloader):
            ...
            for i, (loss_fn, optimizer, scheduler_lr, scheduler_warmup, optimizer_start) in enumerate(
                ...
                stats.update(
                    ...
                    step,
                    epoch,
                    {("lr" if i == 1 else f"lr_{i}"): optimizer.param_groups[0]["lr"], **loss_dict},
                )

            stats.update("train", step, epoch, {**weights_dict, "time": timer.get_elapsed_time()})
            ...
            if not debug and step % cfg.ckpt_every == 0 and step > 0:
                utils.save_ckpt_list(
                    checkpoint_dir, model, cfg, optimizers, scheduler_lrs, scheduler_warmups, stats, train_vars
                )
...
def cli(args=None):
    ...
    parser.add_argument("--config-module", type=str, required=True, default="deepsvg.configs.hierarchical_ordered")
    parser.add_argument("--model_name", type=str, required=False, default="deepsvg")
    parser.add_argument("--log-dir", type=str, default="./logs")
    parser.add_argument("--debug", action="store_true", default=False)
    parser.add_argument("--resume", action="store_true", default=False)
    args = parser.parse_args()
    cfg = importlib.import_module(args.config_module).ExperimentConfig()
    experiment_name = args.config_module.split(".")[-1]
...
# %%
if __name__ == "__main__":
    from deepsvg.constants import PROJECT_ROOT

    model_name = "deepsvg"
    config_module = "deepsvg.configs.hierarchical_ordered"
    experiment_name = config_module.split(".")[-1]
    log_dir = str(PROJECT_ROOT / ".logs")
    cfg = importlib.import_module(config_module).ExperimentConfig()
    debug = False
    resume = False
    train(
        cfg,
