...
class SVGTensorDataset(torch.utils.data.Dataset):
    ...
    def _load_tensor(self, icon_id):
        fname = Path(self.data_dir) / f"{icon_id}.pkl"
    ...
    def get(self, idx=0, model_args=None, random_aug=True, id=None, svg: SVG | None = None):
        if id is None:
            idx = idx % len(self.df)
        ...
        if svg is None:
            ...
            t_sep, fillings = (
                svg.to_tensor(concat_groups=False, PAD_VAL=self.PAD_VAL),
                svg.to_fillings(),
            )
...
class SVGFinetuneDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        self,
        original_dataset: SVGTensorDataset,
        svg_list: list[SVG],
        frac: float = 0.5,
        nb_augmentations: int = 20,
    ):
...
def load_dataset(cfg: _BaseExperimentConfig):
    dataset = SVGTensorDataset(
        data_dir=cfg.data_dir,
        meta_filepath=cfg.meta_filepath,
        model_args=cfg.model_args,
        max_num_groups=cfg.max_num_groups,
        max_seq_len=cfg.max_seq_len,
        max_total_len=cfg.max_total_len,
        filter_uni=cfg.filter_uni,
        filter_platform=cfg.filter_platform,
