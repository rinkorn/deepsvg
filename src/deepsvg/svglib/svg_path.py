...
from deepsvg.svglib.geom import Bbox, Point, Radius, union_bbox
...
class SVGPath:
    def __init__(
        self,
        path_commands: list[SVGCommand] = None,
    ...
    def translate(self, vec):
        for geometry in self._get_unique_geoms():
            geometry.translate(vec)
        return self

    def rotate(self, angle):
        for geometry in self._get_unique_geoms():
            geometry.rotate_(angle)
        return self
    ...
    def to_video(self, wrapper, clips=None, svg_commands=None, color="grey"):
        from .svg import SVG
        from .svg_primitive import SVGCircle, SVGLine
    ...
    def smooth(self):
        ...
        knots = [
            self.start_pos,
            *(path_commmand.end_pos for path_commmand in self.path_commands),
        ]
        r = [knots[0] + 2 * knots[1]]
        f = [2]
        p = [Point(0.0)] * (n + 1)
    ...
    def simplify(self, tolerance=0.1, epsilon=0.1, angle_threshold=179.0, force_smooth=False):
        ...
        points = [
            self.start_pos,
            *(path_command.end_pos for path_command in self.path_commands),
        ]
