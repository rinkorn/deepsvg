...
from deepsvg.svglib.svg_command import SVGCommandBezier, SVGCommandLine, SVGCommandMove
...
from .state.state import DrawMode, LoopMode, PlaybackMode, State, ToolMode
from .utils import *
...
class Sidebar(ScrollView):
    ...
    def swap_paths(self, idx1, idx2):
        ...
        path1, path2 = (
            state.draw_viewbox.get_path(idx1),
            state.draw_viewbox.get_path(idx2),
        )
        ...
        self.sidebar.children[id1], self.sidebar.children[id2] = (
            path_layer2,
            path_layer1,
        )
        state.draw_viewbox.children[id1], state.draw_viewbox.children[id2] = (
            path2,
            path1,
        )
        ...
        for command in svg_path.path_commands:
            if isinstance(command, SVGCommandBezier):
                ...
                segment = BezierSegment.line(
                    flip_vertical(command.start_pos.tolist()),
