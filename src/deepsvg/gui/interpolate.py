...
from deepsvg.svgtensor_dataset import SVGFinetuneDataset, load_dataset
...
def finetune_model(project: DeepSVGProject, nb_augmentations=3500):
    ...
    for step, data in enumerate(dataloader):
        ...
        params_dict, weights_dict = (
            cfg.get_params(step, epoch),
            cfg.get_weights(step, epoch),
        )
...
def compute_interpolation(project: DeepSVGProject):
    ...
    for i1, i2 in zip(keyframe_ids[:-1], keyframe_ids[1:]):
        ...
        svgs = interpolate_svg(
            project.frames[i1].svg,
            project.frames[i2].svg,
            n=frames_inbetween,
            ease=False,
        )
